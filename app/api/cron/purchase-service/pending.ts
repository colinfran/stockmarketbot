...
export const addPendingSpreadOrder = async (insert: PendingInsert): Promise<Response<NoData>> => {
  try {
    await withRetry(() =>
      db.insert(pendingSpreadOrders).values({
...
export const fetchPendingSpreadOrders = async (
  ...
  try {
    const rows = await withRetry(() => db.select().from(pendingSpreadOrders).limit(limit))
...
export const updatePendingSpreadOrder = async (
  ...
  try {
    await withRetry(() =>
      db.update(pendingSpreadOrders).set(patch).where(eq(pendingSpreadOrders.id, id)),
    )
    return { success: true }
  } catch (error) {
...
export const deletePendingSpreadOrder = async (id: string): Promise<Response<NoData>> => {
  try {
    await withRetry(() => db.delete(pendingSpreadOrders).where(eq(pendingSpreadOrders.id, id)))
    return { success: true }
  } catch (error) {
