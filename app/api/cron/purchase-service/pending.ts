...
export const addPendingSpreadOrder = async (insert: PendingInsert): Promise<Response<NoData>> => {
  try {
    await withRetry(async () => {
      const { pendingSpreadOrders } = await getCollections()
      await pendingSpreadOrders.insertOne({
...
export const fetchPendingSpreadOrdersByReport = async (
  ...
  try {
    const rows = await withRetry(async () => {
      const { pendingSpreadOrders } = await getCollections()
      return pendingSpreadOrders
        .find({ market_report_id: reportId }, { projection: { _id: 0 } })
        .limit(limit)
        .toArray()
    })
    return { success: true, data: rows as unknown as PendingSpreadOrder[] }
...
export const updatePendingSpreadOrder = async (
  ...
  try {
    await withRetry(async () => {
      const { pendingSpreadOrders } = await getCollections()
      await pendingSpreadOrders.updateOne({ id }, { $set: patch })
    })
    return { success: true }
  } catch (error) {
...
export const deletePendingSpreadOrder = async (id: string): Promise<Response<NoData>> => {
  try {
    await withRetry(async () => {
      const { pendingSpreadOrders } = await getCollections()
      await pendingSpreadOrders.deleteOne({ id })
    })
    return { success: true }
  } catch (error) {
